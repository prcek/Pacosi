...
<? page_content_begin(); ?>
		...
		<div class="post">
				...
				<table width="280">
                    ...
                    if ($rows=mysql_num_rows($result))
                    {
                      while ($record = MySQL_Fetch_Array($result))
                      {
						...
						echo "\t\t\t<td bgcolor=\"".$stateColor[$state]."\" align=center onClick=\"self.location.href='order.php?date=".$termin."&doc_id=".$doc_id."&auth=".$auth."'\"  style=\"cursor:pointer\">".$name."</td>\n";
                        echo "\t\t</td>\n";
                      }
                    }
                    else 
            ...
            if (($rows>0) && (($auth_role==1) || ($auth_role==3)))
            {
                echo "<p class=\"links\">\n";				
